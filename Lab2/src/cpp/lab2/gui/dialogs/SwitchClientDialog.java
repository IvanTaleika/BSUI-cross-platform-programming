...
public class SwitchClientDialog {
    ...
    public SwitchClientDialog() {
        ...
        dialog.add(new JLabel("New client:"), new GBC(0, 0, 2, 1).setWeight(100, 0).setAnchor(GBC.WEST).setInsets(DEFAULT_INSETS));
        ...
        dialog.add(moneyField, new GBC(1, 2, 1, 1).setWeight(100, 0).setFill(GBC.BOTH).setInsets(DEFAULT_INSETS));

        dialog.add(new JLabel("Type:"), new GBC(0, 3));

        typeComboBox = new JComboBox<String>(ClientTypes);
        dialog.add(typeComboBox, new GBC(1, 3, 1, 1).setWeight(100, 0).setFill(GBC.BOTH).setInsets(DEFAULT_INSETS));
    ...
    private void okButtonListener() {
        ...
        if(name.isEmpty()) {
            JOptionPane.showConfirmDialog(dialog, "Invalid name!", "Error",
